...
Collection.prototype.addBlock = function(id, stats, callback)
{
	...
	else
	{
    	...
    	this._blockchain.getBlockTimes();
...
Collection.prototype.addHistory = function(id, blocks, callback)
{
	...
	else
	{
		...
		// this._blockchain.clean(this.getBestBlockFromItems()); 여기까지

		for (var i = 0; i <= blocks.length - 1; i++)
		{
			this._blockchain.add(blocks[i], id, true);
		};
		// for (var i = 0 ; i <=blocks.length - 1; i++)
		// 	{
		// 		console.log(this._items);d
    // 	}
    //console.log(this._blockchain.);
    console.log("block chain length : "  + this._blockchain._items.length);
    //this._blockchain.ViewHistory();
		this.getCharts();
	}
...
Collection.prototype.setTotalBlockNumber = function(data) //kisu add it
{
	this._blockchain.setTotalBlockNumber(data);
};
Collection.prototype.getTotalBlockNumber = function()
{
	return this._blockchain.getTotalBlockNumber();
}

Collection.prototype.getNodeByIndex = function(index)
{
	//console.log('get node by index : ', index);
...
Collection.prototype.getChartsDebounced = function()
{
	...
	if( this._debounced === null) {
		//console.log(_.map(self._blockchain, 'bestBlockNumber' )); //kisu add it
...
Collection.prototype.requiresUpdate = function(id)
{
  ...
  return ( this.canNodeUpdate(id) && (!this._askedForHistory || _.now() - this._askedForHistoryTime > 2*60*1000) );
}

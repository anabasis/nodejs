# 예 : HTML 대시 보드에서 이벤트를 사용하는 검색 컨트롤
이 페이지에서
예 : HTML 대시 보드에서 이벤트를 사용하는 검색 컨트롤
searchcontrolsevents_html.html
searchcontrolsevents.xml
Splunk는 Simple XML 대시 보드를 HTML로 변환하지 않는 것이 좋습니다. 이 문서는 레거시 앱용으로 제공됩니다.
이 예에서는 변환 된 HTML 대시 보드를 사용하여 변경 이벤트를 사용하여 검색 컨트롤을 검색 관리자와 동기화합니다.
...
searchcontrolsevents.xml
<dashboard>
  <label>Search controls, events</label>
  <row>
    <panel>
      <table>
        <search>

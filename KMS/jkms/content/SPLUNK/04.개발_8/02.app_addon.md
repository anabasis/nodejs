...
5. SavedSearch, Eventtype, Transaction, Tag, Fields 추출, 변환, 조회, 검색 명령 및 데이터 모델과 같은 Splunk 지식 개체로 검색을 보강
...
3. 새 추가 페이지에서 새 App의 속성을 입력
    ...
    <tr><td>Visible</td><td>Splunk Web에 App을 표시해야하는지 여부를 나타냄<br/>
    사용자 인터페이스가 포함 된 App이 표시</td></tr>
...
- 시스템 데이터 (XML, HTML 및 대부분의 CONF 파일)를 편집 할 때는을 사용 `http://<host:port>/debug/refresh`.
- JS 파일을 편집 할 때 splunkd에서 캐싱을 비활성화하고을 사용 `http://<host:port>/debug/refresh`.
...
|version|"1.0.0"또는 "2.0beta"와 같은 앱 버전이 포함 된 문자열입니다. Splunkbase에 게시하려는 App의 경우 app.conf 의 버전 번호가 Splunkbase의 버전 번호와 일치해야합니다.|
...
appIcon_2x.png	72 x 72	Splunk 웹 홈 페이지의 App 목록 (고해상도 디스플레이)
appIcon.png	36 x 36	Splunk 웹 홈 페이지의 App 목록 (표준 해상도 표시)
appIconAlt_2x.png	72 x 72	Splunk 막대 및 검색 막대 (고해상도 디스플레이) 용 App 메뉴
appIconAlt.png	36 x 36	Splunk 막대 및 검색 막대의 App 메뉴 (표준 해상도 표시)	
...
데이터 계층 구성
지식 개체 구성
데이터 처리 방법을 지정하도록 App의 데이터 계층을 구성하십시오. App에서 사용할 수있는 데이터, Splunk Enterprise 인스턴스로 가져 오는 방법 및 데이터 저장 방법을 사용자 지정할 수 있습니다.

저장된 검색, 보고서, 이벤트 유형, 필드, 필드 추출, 태그, 조회, 경고, 데이터 모델, 트랜잭션 및 워크 플로 작업을 포함하여 앱 내 지식 개체를 구성합니다.

데이터 계층 구성
기본적으로 모든 구성은 전역 적입니다. 즉, 모든 App에서 사용할 수 있습니다. 구성을 분리하려면 구성을 App의 디렉토리에 두십시오. 앱 디렉토리에 넣은 구성도 앱과 함께 패키지됩니다. 그러나 인덱싱 된 모든 데이터 입력은 항상 다른 App에서 사용할 수 있습니다.

다음 표는 데이터 계층 구성의 몇 가지 예를 제공합니다. 구성 파일에 대한 자세한 내용 은 관리자 매뉴얼의 구성 파일 정보를 참조하십시오 .

데이터 레이어	기술
입력	App의 데이터 입력을 구성하십시오. App에 대해서만 특정 유형의 데이터를 색인화 할 수 있습니다. 예를 들어 웹 개발자가 웹 로그를 한 곳에서 볼 수 있도록 웹 로그를 인덱싱 할 수 있습니다. 자세한 내용은 데이터 가져 오기 매뉴얼을 참조하십시오 .
...
인덱스	App의 데이터를 저장하도록 사용자 지정 인덱스를 구성하면 앱 사용자가 특정 데이터 만 검색 할 수 있도록하는 가장 좋은 방법입니다.
...
지식 개체 구성
아래 표는 App에서 다양한 지식 객체를 사용하는 방법의 예를 제공합니다. 지식 개체 및 구성 세부 정보에 대한 자세한 내용은 지식 관리자 매뉴얼을 참조하십시오 .

지식 객체	기술
저장된 검색 및 보고서	저장된 검색 및 보고서는 중요한 데이터를 동적으로 캡처합니다. App에서이를 대시보드에 표시하거나 Splunk Web의 드롭 다운 메뉴에 추가하여 필요에 따라 실행하십시오. 저장된 검색을 바로 가기로 사용하여 App에로드 한 모든 데이터에서 흥미롭고 관련성있는 검색을 시작하십시오. 저장된 검색은 대시보드를로드 할 때 검색 결과를 이미 사용할 수 있도록 저장된 검색을 실행하고 데이터를 수집하도록 예약 할 수 있으므로 대시보드를 작성할 때 유용합니다.
이벤트 유형	이벤트 유형은 이벤트를 분류하고 공통 특성별로 분류 할 수 있도록하여 검색 결과를 단순화합니다.
필드 및 필드 추출	필드는 이벤트 데이터에 나타나는 이름-값 쌍입니다. Splunk Enterprise는 데이터에서 필드를 자동으로 추출하지만 고유 한 필드 추출을 정의 할 수도 있습니다. 예를 들어, 사용자 정의 필드를 추출하여 결과에 표시하려는 일부 데이터가 App에있을 수 있습니다.
...
앱 내에서 지식 개체를 구성하여 범위를 지정하는 방법과 역할별로 개체에 대한 권한을 설정할지 여부를 지정합니다. 예를 들어 보고서는 소유자 (개인), 하나의 앱 (앱) 또는 모든 앱 (전역)에 대해서만 표시 될 수있는 개체이며, 사용자 역할마다 읽기/쓰기 권한을 부여 할 수 있습니다. 자세한 내용 은 Splunk App에서 객체에 대한 권한 설정을 참조 하십시오 .
...
Splunk Web에서 권한 설정
파일 시스템에서 권한 설정
앱 내의 모든 앱과 개체는 Splunk Enterprise에서 사용자가보고 (읽기 액세스)하고 상호 작용 (쓰기 액세스) 할 수있는 항목을 지정하는 읽기/쓰기 권한 세트에 의해 관리됩니다. 모든 App 및 개체에 대한 역할별로 권한을 부여 할 수 있습니다.

다음은 앱 또는 App의 객체에 대한 권한을 설정하기위한 사용 사례입니다.

사용자가 볼 수있는 개체를 제한하십시오. 예를 들어 경영진 만 볼 수있는 비즈니스 통계 대시보드와 개발 팀만 볼 수있는 오류보고 대시보드를 만들 수 있습니다.
사용자가 볼 수있는 데이터를 제한하십시오. 예를 들어, 운영팀은 syslog 데이터 만 볼 수있는 권한이 있고 개발 팀은 Log4J 및 Apache 데이터 만 볼 수 있습니다. 따라서 각 팀이 액세스해야하는 다양한 유형의 데이터를 보여주는 각 팀마다 특정 App을 만들 수 있습니다. 이러한 사용자가 Splunk Enterprise에 로그인하면 볼 수있는 권한이있는 앱만 표시됩니다.
개체를 만들거나 편집하는 기능을 제한하십시오. 권한에 따라 특정 사용자는 앱 내에서만 개체를 ​​만들거나 편집 할 수 있지만 다른 사용자는 자신의 사용자 역할에서 사용할 수있는 개체를 만들거나 편집 할 수 있습니다. 예를 들어, 사용자가 보고서를 작성하면 $SPLUNK_HOME/etc/users/<user_name> 아래의 사용자 디렉토리에 저장되며 해당 사용자 만 사용할 수 있습니다. App에 대한 쓰기 권한이있는 사용자는 사용자 수준에서 앱 수준으로 개체를 승격시킬 수 있습니다. 승격 된 오브젝트는 파일 시스템에서 사용자 디렉토리에서 $SPLUNK_HOME/etc/apps/<app_name> 앱 디렉토리로 이동합니다 . 그런 다음 해당 앱 내에서 읽기 권한이있는 모든 사용자가 객체를 사용할 수 있습니다.
...
저장을 클릭 하십시오 .
파일 시스템에서 권한 설정
App에서 권한 파일 ( default.meta )을 편집 하여 App의 모든 객체에 대한 읽기/쓰기 권한을 설정하십시오.
...
이 항목은 모든 사람에게 읽기 권한을 부여하고 App의 모든 이벤트 유형에 대한 관리자 및 권한 역할에 대한 쓰기 권한을 제공합니다.

[eventtypes]
access = read : [ * ], write : [ admin, power ]
객체를 전 세계적으로 사용 가능하게 만들기

기본적으로 개체는 만든 앱 내에서만 개체가 표시됩니다. 모든 App에서 객체를 사용할 수있게하려면 default.meta 의 객체 항목에 다음 줄을 추가하십시오 .

export = system
예를 들어 Splunk Enterprise 설치의 모든 App에서 "테스트"App의 모든 이벤트 유형을 볼 수있게하려면 $SPLUNK_HOME/etc/apps/testing/metadata /default.meta에 다음 항목을 추가하십시오 .

...
또 다른 옵션은 StatsD를 사용하여 메트릭을 수집하지만 collectd를 사용하여 HTTP를 통해 Splunk 플랫폼으로 데이터를 보내는 것입니다. 이 방법의 장점은 수집하면 메트릭 데이터에서 차원 형식을 정규화한다는 것입니다. 자세한 내용은 수집 된 지표 가져 오기를 참조하십시오.
...
4. StatsD 데이터에 대한 데이터 입력 설정에 설명 된대로이 소스 유형에 대한 데이터 입력을 작성하고 사용자 정의 소스 유형을 선택하십시오.
...
수집 된 것을 사용하여 메트릭을 보내려면 다음을 수행하십시오.
...
HEC (HTTP Event Collector)는 HTTP 또는 HTTP (Secure HTTP) 프로토콜을 사용하여 Splunk 플랫폼 배포에 응용 프로그램 이벤트를 보낼 수있는 엔드 포인트입니다. 수집 된 구성에 데이터 입력 세부 사항을 사용해야하므로 수집을 설정하기 전에이 데이터 입력을 구성하십시오.
...
5. 소스 선택 페이지에서 이름에 토큰 이름 (예 : "수집 된 토큰")을 입력하십시오.
...
12. 표시되는 토큰 값을 복사하십시오. 수집 된 토큰을 구성해야합니다.
...
데이터 입력을 테스트하기 위해 수집 된 이벤트를 수집 된 JSON 형식의 데이터를 허용하는 / collector / raw REST API 엔드 포인트를 사용하여 메트릭 인덱스로 직접 보낼 수 있습니다. 측정 항목 색인은 고유 한 HEC 토큰과 소스 유형으로 "collectd_http"가있는 HEC 데이터 입력에 지정됩니다.

The following example shows a curl command that sends a collectd event to the index associated with your HEC token:

다음 예제는 수집 된 이벤트를 HEC 토큰과 연관된 인덱스로 보내는 curl 명령을 보여줍니다.
...
메트릭을 수집하려는 시스템의 컴퓨터에 수집 된 에이전트를 설치하십시오.
...
2. 운영 체제의 설치 지시 사항에 따라 수집 된 버전 5.6 이상을 설치하십시오.
...
수집 된 서버는 다른 입력 및 일대 다 수집 된 클라이언트에서 메트릭을 집계하는 데 사용할 수있는 선택적 데몬입니다.
...
collectd.conf 구성 파일에서 플러그인을 구성하여 데이터를 수집하도록 수집 된 클라이언트를 구성하십시오. collectd.conf 파일의 위치는 운영 체제에 따라 다릅니다. 자세한 내용은 수집 된 웹 사이트의 첫 번째 단계 페이지에서 "구성"을 참조하십시오.
...
>> 설치 방법 및 운영 체제에 따라 일부 플러그인을 별도로 설치해야 할 수도 있습니다. 자세한 내용은 수집 된 웹 사이트를 참조하십시오.
...
수집 된 클라이언트가 연결된 서버와 동일한 시스템에없는 경우에만이 플러그인을 사용으로 설정 한 후 기본 구성을 사용하십시오.
...
수집을 시작하려면 수집 된 웹 사이트의 첫 번째 단계 페이지에서 "데몬 시작"에있는 지시 사항을 따르십시오.
...
collectd.conf 파일에서 활성화 된 모든 플러그인에 대한 모듈이 설치되어 있어야합니다. 누락 된 모듈에 대한 오류가 표시됩니다. 사용 가능한 수집 플러그인에 대한 자세한 내용은 수집 된 Wiki 웹 사이트에서 플러그인 테이블을 참조하십시오.
...
- 문제점 해결은 세부 사항은 로그 파일 플러그인에서 사용 가능한 수집 된 로그 파일을 참조하십시오.
...
-Linux에 수집 된 것을 설치하는 경우 yum을 사용하여 사용 가능한 모듈을 나열 할 수 있습니다. 예를 들어,이 CLI 명령을 사용하십시오.
...
- collectd.conf 파일에서 FQDNLookup 설정을 false로 설정하여 도메인 이름의 이름을 알기 쉽게 표시하십시오.

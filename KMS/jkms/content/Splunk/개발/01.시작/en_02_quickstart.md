...
Start Splunk Enterprise, if it isn't already running.
Open Splunk Web from a web browser by navigating to http://localhost:8000, which is the default URL.
Log in to Splunk Enterprise.
On the Splunk Web home page, click the Gears icon next to Apps.
Click Create app.
...
Under Template, select barebones. The barebones template creates:
...
Click the Splunk logo in the upper left corner of Splunk Web to return to the home page.
Click Add Data.
...
Click Next.
For Index, click Create a new index.
...
Navigate back to your app's home page by clicking the Splunk logo in the upper left of the browser window, then click the tile for your app from the list on the left.
The default home page is the apps's Search page.

In the Search box, enter "index=<your index name> | timechart count(to) by mailer", replacing <your index name> with the index you created in Add data. Leave the time span to the right of the search box set to All time.
...
Save this search as a report by clicking Save As, then selecting Report.
For Title, enter "Top recipients by mailer" and optionally add a description.
Click Save, then click Continue Editing.
Recap
...
Return to Splunk Web and click the Reports tab in the navigation bar.
You should see your report along with any other reports for other apps installed on your Splunk Enterprise instance.

Click This App's to view only the reports associated with your app.
...
Click Edit, then click Edit Permissions.
Change Display For to App.
...
From the app in Splunk Web, click the Reports tab.
Next to the report you created earlier, click Open in Search.
Above the search bar, click Save As, then select Dashboard Panel.
In the Save As Dashboard Panel dialog box, select the following options:
...
Click Save, then click View Dashboard.
...
In a text editor, open $SPLUNK_HOME/etc/apps/your_app/default/data/ui/nav/default.xml.
Add the following line at the end of the <nav> block, replacing the value of "name" with your dashboard's name:
<view name='your_dashboard_name_no_caps_or_spaces' default='true'/>
From line 2 of the file, remove:
default='true'
...
To test that this procedure worked, enter the following command in a new terminal window:
echo $SPLUNK_HOME
...
To test that this procedure worked, enter the following command in a new command window:
echo %$SPLUNK_HOME%
The echo should return the Splunk directory you entered.
...
Install AppInspect. For details, see Install Splunk AppInspect.
...
Run AppInspect again to verify that these issues have been resolved.
...
Next to Search macros click Add new.
Under Destination app make sure your app is selected.
Under Name provide a name for your macro.
...
Click Permissions next to your macro.
Under Object should appear in select This app only.
Click Save to update permissions.
Return to your app by clicking the Apps menu in the upper left corner of the window and selecting your app.
...
On your dashboard, click Edit to open the edit view of your dashboard.


Tip: You can also open edit view by appending "/edit" to your dashboard's URL.

Next to Edit Dashboard, click Source to display the Simple XML view of your dashboard.
In the <query> tag, replace "index=<your_index_name>" with:
...
In a text editor, open $SPLUNK_HOME/etc/apps/your_app_name/metadata/local.meta.
...
In a text editor, open $SPLUNK_HOME/etc/apps/your_app_name/local/macros.conf.
...
Copy the macros.conf file from $SPLUNK_HOME/etc/apps/your_app_name/local folder to the $SPLUNK_HOME/etc/apps/your_app_name/default folder.
In a text editor, open $SPLUNK_HOME/etc/apps/your_app_name/default/macros.conf.
Replace "index=hello_index" with "".
...
Add a comment letting future admins know to add their index specification to this file:

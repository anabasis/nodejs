...
Add-on이 AWS 서비스에서 데이터를 수집하는데 사용할 수 있는 적절한 권한으로 계정 또는 EC2 IAM 역할을 설정할 수 있도록 IAM 역할 및 사용자를 생성할 수 있는 권한이 있어야 함.
...
AWS는 CloudWatch API에 대한 무료 API 호출의 속도를 제한합니다. Splunk는 300회의 기간과 폴링 간격 또는 1800 테스트에서 2백만 개의 메트릭에 대한 데이터 수집 자체가 현재 기본 속도 제한을 초과하지는 않지만 4백만 메트릭 수집은 이를 초과한다는 결론. 자신의 환경에서 수집 할 수백만 개의 측정 항목이 있는 경우 API 제한을 발생 시키려면 비용을 지불하거나 입력에서 덜 중요한 측정 항목을 제거하고 더 많은 세분화를 구성하여 더 적은 API 호출을 만듬.
</td></tr>
...
<table>
<tr><td>고려사항</td><td>특별 참고사항</td></tr>
<tr><td>사이징 및 성능</td><td>AWS는 각 계정을 초당 10개의 요청으로 제한. 각 요청은 1MB의 데이터 만 반환. 즉, 데이터 수집 및 색인 속도는 10MB/s를 넘지 않음. Add-on 모듈 입력은 단일 로그 스트림에서 초당 최대 4,000 개의 이벤트를 처리 할 수 ​​있음.

Best Practice :

- 볼륨이 중요한 경우, 수집하는 실행 기록 데이터의 양을 제한하기 위해 only_after 매개 변수를 구성하십시오.
- 대량 VPC Flow Logs가있는 경우 CloudWatch Logs 입력을 사용하는 대신 Kinesis 입력을 하나 이상 구성하여 수집하십시오.
...
<table>
<tr><td>고려사항</td><td>특별 참고사항</td></tr>
<tr><td>사이징 및 성능</td><td>AWS는 S3 데이터 수집을 버킷 수준에서 조절하므로 모든 데이터가 Splunk 플랫폼에 도착하기 전에 약간의 지연이 예상.
단일 S3 버킷에 대해 여러 개의 S3 입력을 구성하여 성능을 향상시킬 수 있습니다.
Splunk 플랫폼은 각 데이터 입력에 대해 하나의 프로세스를 수행하므로 시스템의 처리 능력이 충분하면 여러 입력으로 성능이 향상. AWS용 Splunk Add-on의 S3 입력에 대한 성능 참조를 참조.</td></tr>
...
<tr><td>AWS 비용</td><td>Billing 리포트 자체에는 요금이 부과되지 않지만 표준 S3 청구가 적용.
...
각 서비스의 마지막 단계로서 Add-on이 서비스의 데이터에 액세스 할 수 있도록 AWS의 Splunk Add-on이 AWS환경에 연결하는데 사용하는 계정 또는 EC2 IAM 역할에 대한 적절한 권한을 구성해야 함. 자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions) 참조.
...
**선행 조건** : 아래의 모든 단계를 수행하려면 관리자가 AWS 계정에 액세스해야 함. 모든 작업을 직접 수행하는데 필요한 권한이 없는 경우 AWS 관리자와 협력하여 AWS의 Splunk Add-on이 연결하는데 사용하는 권한으로 계정 또는 EC2 IAM 역할을 만드는 등의 모든 단계를 완료.
...
2. AWS Config Getting Started 가이드 (<http://docs.aws.amazon.com/config/latest/developerguide/getting-started.html>)를 따라 S3 버킷을 지정하여 데이터 및 SNS 주제를 저장. 구성 알림을 스트림. 기존 버킷이나 SNS를 사용하지 않음. AWS Config 설정에 따라 AWS는 AWS config에 대한 IAM 역할을 자동으로 생성하여 버킷 및 SNS에 필요한 권한을 갖음.
...
6. Add-on이 AWS환경에 연결하는데 사용하는 AWS 계정에 대한 S3 버킷 및 SQS에 액세스 할 수 있는 IAM 권한을 부여. 자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions) 참조.
...
> AWS는 모든 지역의 트레일 데이터를 수집하기 위해 하나의 지역에 글로벌 트레일 또는 하나의 CloudTrail 구성을 제공하지만 SQS 메시지는 이 경우 예상대로 도착하지 않음. 별도의 "CloudTrail S3> SNS> SQS"경로를 구성하여 모든 데이터를 캡처하거나 글로벌 CloudTrail을 구성하려면 아래 3~6 단계를 건너 뛰고 대신 그 S3 버킷에서 직접 데이터를 수집하도록 애드온을 구성
...
2. CloudTrail 이벤트를 저장할 S3 버킷을 만듬. 이 버킷에 대한 권한이 올바른지 AWS 설명서를 참조. <http://docs.aws.amazon.com/awscloudtrail/latest/userguide/create-s3-bucket-policy-for-cloudtrail.html>
...
7. Add-on이 AWS환경에 연결하는데 사용하는 AWS 계정에 대한 S3 버킷 및 SQS에 액세스 할 수 있는 IAM 권한을 부여. 자세한 내용은 AWS 권한 구성을 참조.
...
Inspector는 AWS용 Splunk Add-on을 추가로 구성 할 필요가 없음. 그러나 Amazon Inspector API에 연결하기 위해 Add-on에서 사용하는 AWS 계정 또는 EC2 IAM 역할에 권한을 부여해야 함.
자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions) 참조.

### Kinesis 구성

Kinesis는 AWS용 Splunk Add-on을 추가로 구성 할 필요가 없음. 그러나 AWS 계정이나 Kinesis API에 연결하기 위해 Add-on에서 사용하는 EC2 IAM 역할에 권한을 부여해야 함.
자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions) 참조.

### S3 구성

일반 로그 파일을 수집하는 경우 S3에는 AWS용 Splunk Add-on에 대한 추가 구성이 필요하지 않음. 그러나 Add-on이 S3 버킷에 연결하는데 사용하는 AWS 계정에 권한을 부여해야 함.
자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions) 참조.

액세스 로그를 수집하는 경우 AWS 콘솔에서 로깅을 구성하여 전용 S3 버킷에 로그를 수집해야 함. 액세스 로그를 구성하는 방법에 대한 자세한 내용은 AWS 설명서를 참조.
...
S3 버킷 및 객체를 구성하는 방법에 대한 자세한 내용은 AWS S3 설명서를 참조. <http://docs.aws.amazon.com/gettingstarted/latest/swh/getting-started-create-bucket.html>

### Billing 구성

AWS용 Splunk 앱은 S3 버킷에서 CloudWatch 및 Billing 보고서를 수집하여 Billing 매트릭을 수집.

AWS가 CloudWatch에서 Billing 메트릭을 생성할 수 있게 하려면 청구 및 비용 관리 콘솔의 환경 설정 섹션에서 **수신 경고 알림(Receive Billing Alerts)**을 설정하십시오.

Billing 리포트를 활성화하려면 청구 및 비용 관리 콘솔의 환경 설정 섹션에서 **수신 리포트(Receive Billing Reports)**를 선택하십시오. 대금 청구 및 비용 관리 콘솔에서 S3 버킷을 확인하고 수집 할 보고서 유형을 선택.

AWS용 Splunk Add-on에는 수집하려는 Billing 리포트를 활성화하고 S3 버킷을 수집하여 수집하는 것 이외에 추가 구성이 필요하지 않음. 그러나 Add-on이 AWS환경에 연결하는데 사용하는 AWS 계정에 대한 S3 버킷에 액세스 할 수 있는 권한을 부여해야 함. 자세한 내용은 AWS 권한 구성을 참조.
...
Add-on이 Amazon SNS API에 연결하는데 사용하는 AWS 계정 또는 EC2 IAM 역할에 권한을 부여해야 함. 자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions)을 참조.

SQS 기반 S3 입력을 사용하려면 이벤트가 발생할 때마다 Amazon S3 버킷 이벤트가 SQS 대기열에 알림 메시지를 보내도록해야 함. S3 버킷 이벤트 알림 설정에 대한 지침은 AWS 설명서를 참조.
<https://docs.aws.amazon.com/AmazonS3/latest/UG/SettingBucketNotifications.html>
<http://docs.aws.amazon.com/AmazonS3/latest/dev/NotificationHowTo.html>

### SQS 구성

Add-on이 Amazon SQS API에 연결하는데 사용하는 AWS 계정 또는 EC2 IAM 역할에 권한을 부여해야 함. 자세한 내용은 [Configure AWS permissions](http://docs.splunk.com/Documentation/AddOns/released/AWS/ConfigureAWSpermissions)을 참조.

SQS 기반 S3 입력을 사용하려는 경우 다음을 수행해야 함.
...
메시지가 SQS 기반 S3 입력에 의해 완전히 처리되기 전에 메시지의 가시성 타임 아웃에 도달하면 메시지가 대기열에 다시 나타나고 다시 검색되어 처리되어 중복 데이터가 생성됨.
...
> **Note** : Splunk Web의 Add-on 구성 인터페이스는 SQS 대기열 이름과 S3 버킷을 가져 와서 입력에 사용할 버킷을 선택할 수 있음. 이렇게하려면 관리자가 서비스 계정에 부여하지 않을 GetList 권한이 필요함. 권한 또는 시간 초과로 인해 이러한 찾아보기 이벤트가 실패하는 경우 Splunk 웹 인터페이스나 구성 파일을 통해 수동으로 대기열 및 버킷 이름을 입력 할 수 있음.
...
다음 샘플 정책은 AWS용 Splunk Add-on에 포함 된 10 개의 모든 입력에 필요한 권한을 제공
...
정책의 리소스 섹션에서 데이터를 수집 할 S3 버킷 및 SQS 큐의 ARN을 지정.
...
EC2 리소스에 대한 필수 권한 : **DescribeInstances, DescribeReservedInstances, DescribeSnapshots, DescribeRegions, DescribeKeyPairs, DescribeNetworkAcls, DescribeSecurityGroups, DescribeSubnets, DescribeVolumes, DescribeVpcs, DescribeImages, DescribeAddresses**
...
정책의 리소스 섹션에서 S3 액세스 로그, CloudFront 액세스 로그, ELB 액세스 로그 또는 일반 S3 로그 데이터를 수집 할 S3 버킷의 ARN을 지정.
...
정책의 리소스 섹션에서 계정에 대한 결제 보고서가 포함 된 S3 버킷의 ARN을 지정.

...
<tr><td>Incremental S3</td><td>Incremental S3 input types은 4 개의 AWS 서비스 로그 유형을 수집.
...
<tr><td>Supported log types</td><td>AWS 이외의 사용자 정의 로그를 포함한 모든 로그 유형</td><td>4 가지 AWS 서비스 로그 유형 : CloudTrail 로그, S3 액세스 로그, CloudFront 액세스 로그, ELB 액세스 로그</td><td>5 AWS 서비스 로그 유형 (구성 로그, CloudTrail 로그, S3 액세스 로그, CloudFront 액세스 로그, ELB 액세스 로그) 및 비 AWS 사용자 정의 로그</td></tr>

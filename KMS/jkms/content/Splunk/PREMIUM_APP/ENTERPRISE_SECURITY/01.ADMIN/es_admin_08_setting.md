...
자격 증명 관리에서 기존 인증서의 인증서 텍스트를 편집할 수 있음. 검색 헤드 클러스터에서 인증서를 편집할 수 없습니다.
...
다음 테이블에 있는 매개변수를 사용하여 URL을 수동으로 구성할 수도 있음. 특정 매개변수에 대해 모든 결과를 표시하려면 별표를 사용함. 모든 매개변수가 필요하지는 않음.
...
공백을 HTML로 인코딩하고 주요 이벤트에 표시되는 이름을 콘텐츠 관리에 표시되는 이름 대신 사용함.</td><td>Endpoint - Host With Multiple Infections - Rule</td></tr>
...
Enterprise Security의 대시보드 리스트를 추가 기능별로 보려면 콘텐츠 프로파일 대시보드를 사용함. 콘텐츠 프로파일을 참조.

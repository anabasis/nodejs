...
설정 > 콘텐츠 관리로 이동하고 페이지에서 필터를 사용하여 검색을 봅니다. 이런 저장된 검색을 변경하면 작업 이력 항목이 작업 이력에 더 이상 표시되지 않을 수 있습니다. 작업 이력에서 검색을 제외하려면 작업 이력 검색 추적 허용 리스트 룩업을 사용하십시오. Splunk Enterprise Security에서 룩업 작성 및 관리를 참조하십시오.

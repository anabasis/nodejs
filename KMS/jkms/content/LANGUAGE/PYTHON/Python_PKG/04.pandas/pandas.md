...
- 특정 인덱스 값과 특정 컬럼에 있는 값 가져오기. 이는 `.at`을 이용 가능.
...
- 명시적으로 행이나 열 선택 인자에 : 슬라이스를 전달하면 다음과 같이 행 또는 열 전체
...
- 값 하나를 선택하기 위해서는 특정 행과 열을 지정하는 방식
